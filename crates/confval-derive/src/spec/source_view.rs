...
//! emitted value and field carries its real span through the `_at` constructors,
//! so a location-aware consumer keeps the source positions. The name span and
//! the level container stay detached, because a spec supplies neither.
//!
...
pub(crate) fn field_source_emit(
    ident: &Ident,
    shape: &FieldShape,
    ...
    match shape {
                ...
                quote! {
                    if let ::core::option::Option::Some(__value) = &self.#ident {
                        if !__value.span.is_detached() {
                            __items.push(::confval::format::Field::value_at(
                                #name,
                                __value.span,
                                ::confval::format::Value::at(
                                    __value.span,
        ...
        // A bare list holds no wrapper span, so a source-written empty list is
        // indistinguishable from an absent one and both are omitted. A non-empty
        // list is emitted with each element's own span, and the sequence
        // container span is detached because no wrapper location exists.
        ...
        FieldShape::Nested {
            ...
            quote! {
                if !self.#ident.span.is_detached() {
                    __items.push(::confval::format::Field::block_at(
                        #name,
                        self.#ident.span,
...
fn spanned_string_element() -> TokenStream2 {
    quote! {
        |__element| ::confval::format::Value::at(
            __element.span,
