...
/// named `cli:<key>`, and emitted as an unparsed literal for the leaf parsers
/// to coerce. Arguments that are not `--key=value` flags are ignored, so the
/// caller can pass the whole argument list.
///
