...
/// The usage text, printed to stdout for `--help` and to stderr for a usage
/// error.
    ...
    while let Some(arg) = iter.next() {
        match arg.as_str() {
            "--agent" => {
                ...
                let value = iter.next().ok_or_else(|| missing_value("--agent"))?;
            ...
            "--scope" => {
                ...
                let value = iter.next().ok_or_else(|| missing_value("--scope"))?;
                scope = Some(parse_scope(value)?);
            }
            "--force" => {
                if force {
                    return Err(duplicate("--force"));
                }
                force = true;
            }
            "--launch" => {
                if launch {
                    return Err(duplicate("--launch"));
                }
                launch = true;
            }
            "--list" => {
                if list {
                    return Err(duplicate("--list"));
                }
                list = true;
            }
            other => {
