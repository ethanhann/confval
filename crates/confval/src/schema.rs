...
pub struct SchemaField {
    ...
    ///
    /// `non_empty` is a flag. `unique` below is another flag. Each flag says
    /// whether its rule applies. The optional help line for each is in
    /// `non_empty_help` and `unique_help`. A rule with bounds of its own,
    /// such as a range or a length, is recorded in the [`Constraint`] slot on
    /// [`SchemaType`] instead.
    pub non_empty: bool,
    /// The help line of a `#[confval(non_empty(help = "..."))]` field, or `None`.
    ...
    /// own span.
    pub unique: bool,
    /// The help line of a `#[confval(unique(help = "..."))]` field, or `None`.
    /// The hover reads it after the rule sentence.
