...
The merge reads the parsed tree without the schema.
A repeated block holding one instance in both layers looks the same as a singleton block.
The two instances therefore combine field by field.
