...
See [Templates](/docs/guide/templates) for what populate fills, how a comment is rendered per format, and when emit can fail.
...
Each source is a provider that yields the same neutral `Fields`.
An `Assembly` folds the providers by precedence.
