...
Each frontend and emitter pair sits behind a cargo feature, so a build carries only the formats it enables.
...
Environment and command line values enter as `Unparsed` scalars, so the field's declared type decides what the text becomes.
`Assembly` folds the layers in call order.
`merge` lets a later layer override, and `join` fills only what is missing.
`FromFields` runs once, on the merged result, so a layered spec and a single-file spec are the same type.
...
The `Config` derive writes `Lower`, with an exhaustive destructure of the spec, so a field added on one side without the other is a compile error.
The generated `Lower` impl carries a `Validate + ValidateNested` bound, so a spec without a validator does not compile.
...
A frontend parses through the same confval frontend your program uses, so the diagnostics the editor shows are the ones the program would raise.
