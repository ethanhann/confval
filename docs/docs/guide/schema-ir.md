...
Two more attributes, `#[confval(non_empty)]` and `#[confval(unique)]`, record a precondition as a separate flag.
...
A list can carry `unique` beside `non_empty` and beside a value constraint.
