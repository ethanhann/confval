...
A scalar leaf's default travels rendered to text.
...
A defaulted list, map, or block carries no text, because there is no single value to render, and `has_default` still records that one applies.
