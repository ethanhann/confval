...
confval rejects an unknown field at process startup, so a mistake in a hand-written configuration surfaces as a hard failure rather than a silently ignored key.
