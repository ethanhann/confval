...
This backward direction is called populate, and it gives you two things.
...
It is generated by `#[derive(Spec)]` and lives in the prelude, so `spec.to_template()` works wherever you use confval.
...
A comment is indented to line up with the field it documents, so a comment inside a block sits at the block's indentation:
...
TOML content is flat, so in a TOML template every comment sits at column zero.
